...
#'
#' @return [mom_tbl] with raw shapes and other useful columns
#' @examples
#' x <- bot %>% efourier(6) %>% stat_pca()
...
morphospace.pca <- function(x, xy){
  ...
  for (i in 1:nrow(partition)){
    ...
    # save into res
    # todo vctrs common type bla bla
    res[[i]] <- dplyr::bind_cols(tibble::as_tibble(z), partition_long, xy)
  }

  # bind them all and return
  # suppressWarning since dplyr complaints for mom_tbl attributes
